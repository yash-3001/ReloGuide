...
export default function CreateListingServices() {
    ...
    const params=useParams()
     ...
     useEffect(()=>{
        if(servicelistings && servicelistings.userRef!== auth.currentUser.uid){
            toast.error("You can't edit this listing")
            navigate("/")
        }
    },[navigate,params.servicelistingId]) 
    useEffect(()=>{
        setLoading(true); // see spinner until data is fetched
        async function fetchServiceListing(){
         ...
         if(docSnap.exists()){
             ...
             navigate("/")
             toast.error("Service Listing does not exist")
         }
        }
        fetchServiceListing()
    }, auth.currentUser.uid,navigate,servicelistings)
    ...
    async  function onSubmit(e){
     ...
     //Updating collection in firestore 
        const docRef2 = doc(db,"servicelistings", params.servicelistingId);

        await updateDoc(docRef2, formDataCopy);
        setLoading(false);
  ...
  return (
    <main className='max-w-md px-2 mx-auto'>
         <h1 className='text-3xl text-center mt-6 font-bold'>Edit a Service</h1>
         ...
         <p className='text-lg font-semibold mt-6 text-lime-200'>Name of Service</p>
            ...
            <div className='flex items-center mb-6'>
                ...
                <div className='flex w-full justify-center items-center space-x-6'>
                    ...
                    { ( service_type === "Home-Painting" || service_type ==="BK-Cleaning" || service_type === "Home_Cleaning" || service_type ==="Disinfection" || service_type ==="Pest-Cleaning" ) && (
                    <div className="">
            ...
            <button type="submit" className='mb-6 w-full px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg active:bg-blue-800 active:shadow-lg tansition duration-150 ease-in-out'>Edit Service Listing</button>
        </form>
    </main>
  )
}
